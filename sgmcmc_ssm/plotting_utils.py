...
def _save_plot(fig, save_path, filename, create_dir=True):
    """Helper function to save a plot to a specified path.

    Args:
        fig (matplotlib.figure): Figure to save
        save_path (str): Directory to save the figure in
    ...
    if save_path is not None:
        ...
        logger.info(f"Saved plot to {os.path.join(save_path, filename)}")


def plot_metrics(evaluator, full_trace=True, burnin=None,
                 x='iteration', save_path=None, filename=None):
    ...
    if save_path is not None:
        if filename is None:
            filename = f"metrics_plot_{evaluator.sampler.name}.png"
        _save_plot(g.fig, save_path, filename)
...
def compare_metrics(evaluators, full_trace=True, burnin=None, errorband=False,
    ...
    if errorband:
        ...
        g = sns.FacetGrid(df, col='variable:metric', hue="method",
                          col_wrap=4, sharey=False).map_dataframe(
            sns.lineplot, x=x, y='value',
            units='init', estimator=None, errorbar=None,
        )
    ...
    if save_path is not None:
        if filename is None:
            filename = "compare_metrics_plot.png"
        _save_plot(g.fig, save_path, filename)
...
def plot_trace_plot(evaluator, full_trace=True, query_string=None,
    ...
    if burnin is None:
        if not full_trace:
            samples_df = samples_df.query(
                'iteration >= {0}'.format(evaluator.iteration/2))
    else:
    ...
    if save_path is not None:
        if filename is None:
            filename = f"trace_plot_{evaluator.sampler.name}.png"
        _save_plot(fig, save_path, filename)
...
def plot_svm_data_fit(observations, true_latent_vars=None,
                      ...
                      ignore_warning=False, save_path=None, filename=None):
    ...
    if sampler is not None:
        ...
        axes[1].plot(smoothed_mean[:, 0] +
                     np.sqrt(smoothed_var[:, 0, 0]), '--C2')
        axes[1].plot(smoothed_mean[:, 0] -
                     np.sqrt(smoothed_var[:, 0, 0]), '--C2')
