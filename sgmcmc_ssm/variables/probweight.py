    ...
    def set_var(self, param, **kwargs):
        if self._logit_name in kwargs:
            if np.size(kwargs[self._logit_name]) != 1:
                raise ValueError(
                    "{} must be 1D scalar".format(self._logit_name))
    ...
    def set_var(self, param, **kwargs):
        if self._logit_name in kwargs:
            ...
            if num_states != num_states2:
                raise ValueError(
                    "{} must be square matrix".format(self._logit_name))
            param.var_dict[self._logit_name] = np.array(
                kwargs[self._logit_name]).astype(float)
            ...
            if num_states != num_states2:
                raise ValueError(
                    "{} must be square matrix".format(self._expanded_name))
            param.var_dict[self._expanded_name] = np.array(
                kwargs[self._expanded_name]).astype(float)
            param._set_check_dim(**{self.dim_names[0]: num_states,
