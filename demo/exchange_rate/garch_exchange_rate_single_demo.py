...
from sgmcmc_ssm.models.garch import GARCHSampler, SeqGARCHSampler
import os
import numpy as np
import pandas as pd
...
# Initialize GARCH sampler
# Parameters for GARCH(1,1) model:
# alpha: ARCH parameter (impact of previous squared returns)
# beta: GARCH parameter (persistence of volatility)
# gamma: leverage parameter (asymmetric impact of positive vs negative returns)
# tau: scale parameter for observation noise
sampler = GARCHSampler(n=1, m=1, observations=split_observations[1])
sampler.prior_init()
...
def compare_smoothed_pfs(sampler, list_of_kwargs):
    ...
    for ii, (name, (x_mean, x_cov)) in enumerate(means_covs.items()):
        ...
        ax.plot(x_mean-np.sqrt(x_cov), "--C{}".format(ii), alpha=0.5)
    ax.plot(observations**2, '.k', alpha=0.3, label='data^2')
    ax.legend()
    return fig, ax
...
fig, ax = compare_smoothed_pfs(sampler, list_of_kwargs)
...
# For GARCH, we want to sample the log_mu, logit_phi, logit_lambduh and tau parameters
sample_functions = sample_function_parameters(
    ['log_mu', 'logit_phi', 'logit_lambduh', 'LRinv'])
...
# Plot actual data fit
plot_garch_data_fit(sampler.observations, sampler=sampler, tqdm=tqdm)

# Plot parameter traces and metrics
